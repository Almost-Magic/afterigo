...
def _get_kokoro():
    """Lazy-load Kokoro-82M TTS pipeline if installed."""
    global _kokoro_pipeline, _kokoro_checked
    if _kokoro_checked:
    ...
    try:
        ...
        logger.info("Kokoro-82M TTS loaded (local fallback)")
    except ImportError:
        logger.info("Kokoro-82M not installed — browser TTS will be final fallback")
    except Exception as exc:
        logger.warning("Kokoro-82M init failed: %s", exc)
    return _kokoro_pipeline


def _kokoro_synthesise(text: str) -> bytes | None:
    """Synthesise text to WAV bytes via Kokoro-82M. Returns None on failure."""
    ...
    try:
        ...
        import struct
        samples = []
        for _, _, audio_chunk in pipeline(text, voice="af_heart"):
        ...
        logger.info("Kokoro TTS: %d bytes for %d chars", buf.tell(), len(text))
        return buf.getvalue()
    except Exception as exc:
...
def create_chat_routes():
    ...
    def tts_status():
        """Report voice capability — ElevenLabs → Kokoro → browser."""
        has_key = bool(ELEVENLABS_API_KEY)
        has_kokoro = _get_kokoro() is not None
        if has_key:
            active = "elevenlabs"
        elif has_kokoro:
            active = "kokoro"
        ...
        return jsonify({
            ...
            "fallback_chain": ["elevenlabs", "kokoro", "browser-tts"],
            "setup": None if has_key else (
                "Set ELEVENLABS_API_KEY environment variable or add it to CK/elaine/.env "
                "for premium voice. Kokoro-82M provides free local fallback."
            ),
        })
    ...
    def tts():
        """Convert text to speech. Fallback chain: ElevenLabs → Kokoro-82M → browser hint.
        Body: {"text": "..."} — max 500 chars for chat responses.
        Returns audio/mpeg (ElevenLabs), audio/wav (Kokoro), or JSON hint for browser TTS."""
        data = request.get_json(silent=True) or {}
        ...
        # ── Try 1: ElevenLabs (premium cloned voice) ─────────────
        if ELEVENLABS_API_KEY:
            try:
                ...
                logger.warning("ElevenLabs TTS failed, trying Kokoro: %s", e)

        # ── Try 2: Kokoro-82M (local neural TTS) ─────────────────
        wav_data = _kokoro_synthesise(text)
        if wav_data:
            return Response(wav_data, mimetype="audio/wav")

        # ── Try 3: Signal browser to use client-side TTS ─────────
        return jsonify({
            "fallback": "browser-tts",
            "text": text,
            "message": "No server-side TTS available. Use browser speechSynthesis.",
        })
