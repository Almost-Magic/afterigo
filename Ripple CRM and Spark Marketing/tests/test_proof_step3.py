...
def test_deals_pipeline_page(page):
...
def test_interactions_timeline_page(page):
...
def test_commitments_page(page):
...
def test_contact_detail_timeline(page):
    ...
    # Verify Activity Timeline section exists (default tab)
    assert page.locator("text=Activity Timeline").first.is_visible()
    assert page.locator("text=Log Interaction").first.is_visible()

    # Switch to Notes tab and verify
    page.locator("button:has-text('Notes')").first.click()
    page.wait_for_timeout(500)
